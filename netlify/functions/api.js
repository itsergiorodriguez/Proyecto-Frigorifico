...
exports.handler=async function(event){
  ...
  try{
    ...
    if(m==='GET'&&p[0]==='usuarios'){if(!s){const r=await pool.query('SELECT nombre FROM usuarios ORDER BY nombre');return ok(r.rows.map(x=>x.nombre));}if(s.rol!=='admin')return err('Sin permiso',403);const{rows}=await pool.query('SELECT id,nombre,rol FROM usuarios ORDER BY nombre');return ok(rows);}
